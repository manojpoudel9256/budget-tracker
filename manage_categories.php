...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'add') {
    ...
    if (!empty($name)) {
        ...
        if ($stmt->execute([$user_id, $name, $type, $color])) {
            ...
            $message = $lang['error_adding_category'];
            $msg_type = "danger";
        }
    }
}
...
<div class="container-fluid px-0 px-md-3 pb-5">
    ...
    <div class="px-3 mb-4 mt-2 fade-in-up">
        ...
        <p class="text-muted small fw-medium"><?php echo $lang['customize_labels']; ?></p>
    </div>
    ...
    <div class="row px-3 g-4">
        ...
        <div class="col-12 col-lg-4 order-lg-2 mb-4">
            <div class="glass-card p-4 fade-in-up delay-1 sticky-lg-top" style="top: 100px; z-index: 1;">
                <h5 class="fw-bold mb-4"><i
                        class="fas fa-plus-circle me-2 text-primary"></i><?php echo $lang['create_new']; ?></h5>
                ...
                <form method="POST">
                    ...
                    <div class="bg-light p-1 rounded-3 d-flex mb-3">
                        ...
                        <label class="btn btn-sm border-0 flex-fill rounded-3 fw-bold text-muted" for="typeExp"
                            onclick="toggleType('expense')"><?php echo $lang['expense']; ?></label>
                        ...
                        <label class="btn btn-sm border-0 flex-fill rounded-3 fw-bold text-muted" for="typeInc"
                            onclick="toggleType('income')"><?php echo $lang['income']; ?></label>
                    </div>

                    <div class="form-floating mb-3">
                        <input type="text" name="name" class="form-control rounded-4 border-0 bg-light" id="catName"
                            placeholder="<?php echo $lang['category_name']; ?>" required>
                        <label for="catName" class="text-muted"><?php echo $lang['category_name']; ?></label>
                    </div>

                    <div class="mb-4">
                        <label
                            class="form-label text-muted small fw-bold text-uppercase"><?php echo $lang['color_tag']; ?></label>
                        <div class="d-flex align-items-center gap-2">
                            <input type="color" name="color"
                                class="form-control form-control-color border-0 rounded-circle p-0 shadow-sm"
                                value="#6366f1" title="<?php echo $lang['pick_color']; ?>"
                                style="width: 40px; height: 40px; cursor: pointer;">
                            <span class="text-muted small ms-2"><?php echo $lang['pick_color']; ?></span>
                        </div>
                    </div>

                    <button type="submit" class="btn w-100 text-white fw-bold rounded-4 shadow-sm py-3"
                        style="background: var(--primary-gradient);">
                        <i class="fas fa-check me-2"></i> <?php echo $lang['save_category']; ?>
                    </button>
                </form>
            </div>
        </div>
        ...
        <div class="col-12 col-lg-8 order-lg-1">
            ...
            <div class="glass-card p-0 mb-4 fade-in-up delay-2 overflow-hidden">
                <div class="p-4 border-bottom bg-white bg-opacity-50">
                    <h5 class="fw-bold mb-0 text-danger"><i
                            class="fas fa-arrow-up me-2"></i><?php echo $lang['expense_categories']; ?></h5>
                </div>
                <div class="list-group list-group-flush">
                    <?php if (empty($expense_cats)): ?>
                        <div class="p-4 text-center text-muted"><?php echo $lang['no_custom_expense']; ?></div>
                    <?php else:
                        foreach ($expense_cats as $cat): ?>
                            <div
                                ...
                                <a href="?delete=<?php echo $cat['id']; ?>"
                                    ...
                                    onclick="return confirm('<?php echo $lang['confirm_delete_category']; ?>')"
            ...
            <div class="glass-card p-0 mb-4 fade-in-up delay-3 overflow-hidden">
                <div class="p-4 border-bottom bg-white bg-opacity-50">
                    <h5 class="fw-bold mb-0 text-success"><i
                            class="fas fa-arrow-down me-2"></i><?php echo $lang['income_categories']; ?></h5>
                </div>
                <div class="list-group list-group-flush">
                    <?php if (empty($income_cats)): ?>
                        <div class="p-4 text-center text-muted"><?php echo $lang['no_custom_income']; ?></div>
                    <?php else:
